...
- `index.html` contains the page structure and all top-level sections from the PDF.
- `styles.css` contains all responsive layout and visual styling.
- `main.js` contains all rendering logic for curriculum items, course cards, course details, filters, and projects.
...
```js
{
  ...
  syllabus: ["..."],
  outcomes: ["..."],
  instructors: [{ name: "Pavel Mavrin", image: "images/mavrin.jpeg" }],
...
Use the repository root as the Pages source. The site has no build step and no external runtime dependencies.
